// source code		: https://github.com/Ivaneo/threadpool/blob/master/threadpool.h
// description		: https://habrahabr.ru/post/188234/
// description copy	: ./Manuals/Пишем простой ThreadPool с async и future.ntml
...
public:
    ...
    std::shared_ptr<Result<_R>> runAsync(_FN _fn, _ARGS... _args)
    {
        std::function<_R()> rfn = std::bind(_fn,_args...);
        std::shared_ptr<Result<_R>> pData(new Result<_R>());
        fn_type fn = [=]()
        {
            pData->Data = rfn();
            pData->Ready = true;
